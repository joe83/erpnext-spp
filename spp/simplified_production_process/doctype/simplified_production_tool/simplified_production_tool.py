			...
			if se:
				...
				for d in se.items:
					...
					if  d.s_warehouse and not allow_negative_stock and d.actual_qty < d.transfer_qty:
						frappe.throw(_("For manufacturing item {0} you need {3} x {2} in warehouse {1}").format(i['manufacture_item'],
							frappe.bold(d.s_warehouse),  frappe.bold(d.item_code), frappe.bold(d.transfer_qty))
							+ '<br>' + _("Available qty is {0}, you need {1}").format(frappe.bold(d.actual_qty),frappe.bold(d.transfer_qty)),
